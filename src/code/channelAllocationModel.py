...
def channelAllocationRandom(Edge,enQ,dataQ,virtualQ,links,chCap,batterCapacity,P_R,chState,t):
    caResults = np.zeros((numOfL,numOfCH),dtype=np.int8)
    # 为每个颜色寻找最大带权独立集
    # print("t:",t,"信道容量",chCap)
    for k in range(numOfCH):
        ...
        while(np.size(G.V) > 0 and np.sum(wEdgeDegree) > 0):
            ...
            wEdgeDegree[linkWithConflict] = wEdge[linkWithConflict]/(G.dList[linkWithConflict]+1)
            wEdgeDegree[hasDeleted] = 0
            # 随机选择wEdgeDegree中权重不为零的链路分配信道 k
            choosen = randomChoose(wEdgeDegree)
            if np.max(wEdgeDegree) != 0:
                # print("为链路",choosen,"分配信道",k)
                caResults[choosen,k] = 1
                # 删除结点 choosen 以及与其邻接的所有结点
...
def chooseMax(wEdgeDegree):
    idx = np.where(wEdgeDegree == np.max(wEdgeDegree))
    choosen = idx[0][0]
    return choosen

def channelAllocationOptimal(Edge,enQ,dataQ,virtualQ,links,chCap,batterCapacity,P_R,chState,t):
    caResults = np.zeros((numOfL,numOfCH),dtype=np.int8)
    # 为每个颜色寻找最大带权独立集
    # print("t:",t,"信道容量",chCap)
    maxWIS = - inf
    for k in range(numOfCH):
        ...
        while(np.size(G.V) > 0 and np.sum(wEdgeDegree) > 0):
            ...
            wEdgeDegree[hasDeleted] = 0
            # 寻找最大权独立集
            noZeros = np.where(wEdgeDegree != 0)
            for v in noZeros[0]:
                choosen = v
                if np.max(wEdgeDegree) != 0:
                    # print("为链路",choosen,"分配信道",k)
