...
# 初始化结点能量队列
enQ = np.zeros((numOfN, timeSlots, len(epsilons)))
...
virtualQ = np.zeros((numOfN, timeSlots, len(epsilons)))
    ...
    for e in range(len(epsilons)):
        ...
        for w in range((len(weights))):
            ...
            for t in range(timeSlots - 1):
                ...
                caResults = channelAllocation(Edge, enQ, dataQ, virtualQ, link, channelCapacity,
                                              enQ_max[w], P_R, chState, P_max, "random", t)
            ...
            # aveUtility[w] = np.sum(np.log(1 + aveHar)) - np.sum(np.log(1 + aveDrop))
            ...
            # print("aveHar",aveHar,"aveDrop", aveDrop)
            # print("aveDrop", aveDrop)
            print("w = ", weights[w], "RA :", aveUtility[w])
    np.savetxt('E:\\utilityCompare_1.csv', aveUtility, delimiter=',')
    np.savetxt('E:\\trafficOverSlot_1.csv', trafficOverSlot, delimiter=',')
    np.savetxt('E:\\numOfCA_1.csv', numofCA, delimiter=',')
    np.savetxt('E:\\dataDrop_1.csv', dropMw, delimiter=',')
